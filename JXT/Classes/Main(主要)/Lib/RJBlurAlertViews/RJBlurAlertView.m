...
static const CGFloat RJAlertViewDefaultTextFontSize = 20;
...
{
    _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.alertView.frame.size.width-40, RJAlertViewTitleLabelHeight)];
    ...
    if (self.contentType == RJBlurAlertViewContentTypeText) {
        _textLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.alertView.frame.size.width-40, self.contentSize.height)];
        _textLabel.center = CGPointMake(self.alertView.frame.size.width/2, CGRectGetHeight(self.alertView.frame)/2 -40);
        _textLabel.text = text;
//        _titleLabel.textColor = JWColor(54, 55, 56);
...
{
    UIView * alertSquare = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, RJAlertViewTitleLabelHeight+2*RJAlertViewSpaceHeight+self.contentSize.height+RJAlertViewTitleLabelHeight-10)];
    alertSquare.backgroundColor = [UIColor whiteColor];
    ...
    UIView *topBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(alertSquare.frame), 50)];
    topBackgroundView.backgroundColor = JWColor(246, 246, 246);
...
- (void)_buttonSetupWithCancelButton:(BOOL) hasCancelButton
{
    ...
    if (hasCancelButton) {
        //ok button
        _okButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width /2, 60)];
        _okButton.center = CGPointMake((CGRectGetWidth(self.alertView.frame)/4)+3, buttonCenterY -10);
        
        //cancel button
        _cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width /2, 60)];
        _cancelButton.center = CGPointMake((CGRectGetWidth(self.alertView.frame)*3/4)-3, buttonCenterY-10);
        ...
        _okButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, self.alertView.frame.size.width, 60)];
        _okButton.center = CGPointMake(CGRectGetWidth(self.alertView.frame)/2, buttonCenterY -10);
    }
    
    [_okButton setBackgroundColor:JWColor(92, 173, 89)];
    
    //ok button end setup
    [_okButton setTitle:@"确定" forState:UIControlStateNormal];
    _okButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:20.0f];
